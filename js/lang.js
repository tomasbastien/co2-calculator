...
var changelog_langEn = {    
    ...
    'v1.2.0-release-date' : '   04th Fev, 2023',
    ...
    'v1.2.1-release-date' : '   04th Fev, 2023',
    ...
    'v2.0.0-release-date' : '   8th Mar, 2023',
    ...
    'v2.1.0-release-date' : '   21th Mar, 2023',
    ...
    'v2.2.0-release-date' : '   24th Apr, 2023',
    ...
    'v2.2.1-release-date' : '   29th Apr, 2023',
...
var changelog_langFr = {    
    ...
    'v1.0.0-release-notes' : '<strong>Première version et publication</strong>',
...
var co2calculator_langEn = { 
  ...
  'how-does-this-tool-work' : 'Based on provided geolocations, here\'s the operation mode to provide an <u>estimated</u> CO2e emission for the specified trip :<ul class=""> </br>   <li>Each location string is sent to <strong>Nominatim Openstreetmap</strong> to query the OSM database for [lat,lon] GPS coordinates then stored in a list.</li> <ul>        <li><em>If GPS coordinates are directly provided (whether DD : ex: 45.8994, 6.1269 or DMS format : ex: 45°53\'57.8"N 6°07\'36.8"E), the tools skips the location query on Nominatim.</em></li>        <li><em>This tool version takes the very first result of the query, so provided geolocation has to be the most accurate possible (using local state, city or country). It will be enhanced later to provide results list to the user.</em></li> </ul>   </br>   <li>This [lat,lon] GPS coordinates list is sent over a different routing API based on chosen transportation mode :  </li>   <ul>    <li><strong>Openrouteservice directions API</strong> for any "road" mode    , in order to plan a trip throught all these locations and obtain kilometers journey length and route as a GPS points array.</br><em>Openrouteservice directions API only supports transportation profiles driving-car and regular-bike, so public transportations may required to specify steps to match with the local public transportation plan. This could be also be a future enhancement.</em></li>  <li><strong>A simple crowfly</strong> math calculation for plane, this needs to declare as many steps as possible</li>  <li><strong>BRouter.de</strong> for trains journey, that sticks as much as possible to railway path and need to have departure and arrival on an existant train station</li>    <!-- <li><strong>SNCF journey API</strong> that will mix regular and high-speed train for the France area</li> -->  </ul>   </br>   <li>Total kilometers is sent to <strong>Mon Impact Transport API</strong> (provided by ADEME, FR gov. ecologic transition agency), with chosen transportation mode. It returns kgco2e emissions, then divided by carpoolers if specified.</li>  <ul>        <li><em>Default passengers number is 1 for any other transportation than car and electric car, as estimation returned from ADEME for buses assumes an average transportation occupancy.</em></li>    </ul>   </br>   <li>GPS points array is shown as a line over a <strong>LeafLet</strong> Map, centered to fit global itinerary within the tile. This is usefull to see if the tool actually match with expected or realized journey.</li></ul><div class="alert alert-dark" role="alert">  No data is stored on the server, all computation is done client-side by the  web-browser. All is HTML5/Javascript written and prettified with Bootstrap 5 CSS library. Some functions (GPS coord. validation) have been AI-written using <strong>ChatGPT</strong> prompts.</div>All recensed bugs and futur features can be find on <a href="./changelog.html" class="link-dark">changelog page</a>.',
...
var co2calculator_langFr = { 
  ...
  'how-does-this-tool-work' : 'En fonction des géolocalisations fournies, voici le mode opératoire pour fournir une estimation de l\'émission de CO2e pour le trajet spécifié :<ul class=""></br><li>Chaque chaîne de localisation est envoyée à <strong>Nominatim Openstreetmap</strong> pour interroger la base de données OSM afin d\'obtenir les coordonnées GPS [lat,lon] alors stockées dans une liste.</li><ul><li><em>Si les coordonnées GPS sont directement fournies (que ce soit au format DD : ex: 45,8994, 6,1269 ou DMS : ex: 45°53\'57.8"N 6°07\'36.8"E), l\'outil ne lance pas la requête vers Nominatim.</em></li><li><em>Cette version de l\'outil prend le tout premier résultat de la requête, donc la géolocalisation fournie doit être la plus précise possible (en utilisant la région, la ville ou le pays). Elle sera améliorée ultérieurement pour fournir une liste de résultats à l\'utilisateur.</em></li></ul></br><li>Cette liste de coordonnées GPS [lat,lon] est envoyée sur une API de routage différente en fonction du mode de transport choisi :</li><ul><li><strong>Openrouteservice directions API</strong> pour tout mode de transports sur route, afin de planifier un voyage à travers toutes ces localisations et d\'obtenir la longueur du trajet en kilomètres et l\'itinéraire sous forme d\'un tableau de points GPS.</br><em>L\'API d\'Openrouteservice ne prend en charge que les profils de transport "voiture" et "vélo classique", les transports en commun peuvent donc nécessiter de spécifier des étapes pour correspondre au tracé de transport en commun. Cela pourrait également être une amélioration dans le futur.</em></li><li><strong>Un simple calcul mathématique à vol d\'oiseau</strong> pour les avions, nécessitant de fournir autant d\'étapes que possible.</li><li><strong>BRouter.de</strong> pour les trajets en train, qui suit autant que possible les chemins de fer et doit avoir un départ et une arrivée dans une gare existante.</li><!-- <li><strong>SNCF journey API</strong> qui mélangera trains réguliers et à grande vitesse pour la région France</li> --></ul></br><li>Le nombre total de kilomètres est envoyé à <strong>Mon Impact Transport API</strong> (fourni par l\'ADEME, l\'agence de transition écologique du gouvernement français), avec le mode de transport choisi. Cette dernière renvoie les émissions de CO2 (en kgco2e), puis les divise par le nombre de passagers si spécifié.</li><ul><li><em>Le nombre de passagers par défaut est "1" pour tout transport autre que "voiture" et "voiture électrique", car l\'estimation retournée par ADEME pour les autres profils suppose une occupation moyenne des transports.</em></li></ul></br><li>Le tableau de points GPS est affiché sous forme de ligne sur une carte <strong>LeafLet</strong>, centrée pour s\'adapter à l\'itinéraire global dans la fenêtre. Cela est utile pour voir si le résultat de l\'outil correspond effectivement au voyage prévu ou réalisé.</li></ul><div class="alert alert-dark" role="alert">    Aucune donnée n\'est stockée sur le serveur, tous les calculs sont effectués côté client par le navigateur Web. Tout est écrit en HTML5/Javascript et est embelli avec la bibliothèque CSS Bootstrap 5. Certaines fonctions (validation des coordonnées GPS) ont été écrites par intelligence artificielle en utilisant des prompts <strong>ChatGPT</strong>.</div>Tous les bugs recensés et les fonctionnalités futures peuvent être trouvés sur <a href="./changelog.html" class="link-dark">la page de changelog</a></div>',
...
function updateLanguage(page) {
  ...
  // document.getElementById('v1.0.0-release-date').innerText = lang['v1.0.0-release-date'];
  // document.getElementById('v1.0.0-release-notes').innerHTML = lang['v1.0.0-release-notes'];
}
